...
오류가 발생했을 때 오류 자체에 집중하기보다, **어떤 흐름과 판단에서 비롯된 오류인지 먼저 파악**한다.
근본 원인을 이해한 뒤 수정하고, 같은 실수가 반복되지 않도록 스스로 작업 방식을 교정한다.
"확인했다", "문제없다", "되어 있다"는 표현은 실제로 확인한 경우에만 사용한다.

오류가 해결되고 교정까지 완료된 경우, `docs/claude/error-log.md`에 간략하게 기록한다.
형식: `YYYY-MM-DD` + 어떤 판단 실수였는지 한 줄 + 교정한 방향 한 줄.
